...
WebMidi.enable(function () {
    ...
    input.addListener('noteon', 1,
        function (e) {
            console.log("MIDI Note: (" + e.note.name + e.note.octave + ").");
            inst.toggleSound( e.rawVelocity, e.note.name + String(e.note.octave) );
        }
    );
  ...
  */
  ...
  class Instrument {
    ...
    toggleSound(velocity, note) {
      let method = velocity === 127 ? 'triggerAttack' : 'releaseAll';
      switch (note) {
          case 'C4':
              this.synth[method](['C4', 'E4', 'G4']);
              break;
          case 'D4':
              this.synth[method](['D4', 'F4', 'A4']);
              break;
          case 'E4':
              this.synth[method](['E4', 'G4', 'B4']);
              break;
          case 'F4':
              this.synth[method](['F4', 'A4', 'C5']);
              break;
          case 'G4':
              this.synth[method](['G4', 'B4', 'D5']);
              break;
          case 'A4':
              this.synth[method](['A4', 'C5', 'E5']);
              break;
          case 'B4':
              this.synth[method](['B4', 'D5', 'F5']);
              break;
          case 'C5':
              this.synth[method](['C5', 'E5', 'G5']);
              break;
      }
    }
